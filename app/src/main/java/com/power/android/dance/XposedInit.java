package com.power.android.dance;
...
import com.power.android.dance.callbacks.XC_InitPackageResources;
import com.power.android.dance.callbacks.XC_LoadPackage;
import com.power.android.dance.callbacks.XCallback;
import com.power.android.dance.services.BaseService;

import static com.power.android.dance.DreamBridge.hookAllConstructors;
import static com.power.android.dance.DreamBridge.hookAllMethods;
import static com.power.android.dance.XposedHelpers.callMethod;
import static com.power.android.dance.XposedHelpers.closeSilently;
import static com.power.android.dance.XposedHelpers.fileContains;
...
import static com.power.android.dance.XposedHelpers.findFieldIfExists;
import static com.power.android.dance.XposedHelpers.getBooleanField;
import static com.power.android.dance.XposedHelpers.getObjectField;
import static com.power.android.dance.XposedHelpers.getOverriddenMethods;
import static com.power.android.dance.XposedHelpers.getParameterIndexByType;
import static com.power.android.dance.XposedHelpers.setObjectField;
import static com.power.android.dance.XposedHelpers.setStaticBooleanField;
import static com.power.android.dance.XposedHelpers.setStaticLongField;
import static com.power.android.dance.XposedHelpers.setStaticObjectField;

/*package*/ final class XposedInit {
	private static final String TAG = DreamBridge.TAG;

	private static final boolean startsSystemServer = DreamBridge.startsSystemServer();
	private static final String startClassName = DreamBridge.getStartClassName();
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			XC_MethodHook callback = new XC_MethodHook() {
				...
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
					DreamBridge.closeFilesBeforeForkNative();
				}

				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					DreamBridge.reopenFilesAfterForkNative();
				}
			};
		...
		findAndHookMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new XC_MethodHook() {
			...
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				...
				XC_LoadPackage.LoadPackageParam lpparam = new XC_LoadPackage.LoadPackageParam(DreamBridge.sLoadedPackageCallbacks);
			...
			findAndHookMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new XC_MethodHook() {
						...
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							...
							lpparam.classLoader = DreamBridge.BOOTCLASSLOADER;
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", DreamBridge.BOOT_START_TIME);
		}
	...
	/*package*/ static void hookResources() throws Throwable {
		...
		if (!DreamBridge.initXResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			DreamBridge.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}
		...
		hookAllConstructors(TypedArray.class, new XC_MethodHook() {
			...
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
				...
				if (res instanceof XResources) {
					DreamBridge.setObjectClass(typedArray, XResources.XTypedArray.class);
				}
			}
		});

		// Replace system resources
		XResources systemRes = (XResources) DreamBridge.cloneToSubclass(Resources.getSystem(), XResources.class);
	...
	private static XResources cloneToXResources(XC_MethodHook.MethodHookParam param, String resDir) {
		...
		XResources newRes = (XResources) DreamBridge.cloneToSubclass(result, XResources.class);
		newRes.initObject(resDir);

		// Invoke handleInitPackageResources().
		if (newRes.isFirstLoad()) {
			String packageName = newRes.getPackageName();
			XC_InitPackageResources.InitPackageResourcesParam resparam = new XC_InitPackageResources.InitPackageResourcesParam(DreamBridge.sInitPackageResourcesCallbacks);
	...
	private static void hookXposedInstaller(ClassLoader classLoader) {
		try {
			findAndHookMethod(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "getActiveXposedVersion",
					XC_MethodReplacement.returnConstant(DreamBridge.getXposedVersion()));
	...
	/*package*/ static void loadModules() throws IOException {
		...
		ClassLoader topClassLoader = DreamBridge.BOOTCLASSLOADER;
		ClassLoader parent;
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(DreamBridge.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, DreamBridge.BOOTCLASSLOADER);
				...
				try {
					...
					if (DreamBridge.isZygote) {
						...
						if (moduleInstance instanceof IXposedHookLoadPackage)
							DreamBridge.hookLoadPackage(new IXposedHookLoadPackage.Wrapper((IXposedHookLoadPackage) moduleInstance));

						if (moduleInstance instanceof IXposedHookInitPackageResources)
							DreamBridge.hookInitPackageResources(new IXposedHookInitPackageResources.Wrapper((IXposedHookInitPackageResources) moduleInstance));
					} else {
