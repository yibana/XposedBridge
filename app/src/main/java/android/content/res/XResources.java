...
import com.power.android.dance.DreamBridge;
import com.power.android.dance.IXposedHookZygoteInit;
import com.power.android.dance.XC_MethodHook;
import com.power.android.dance.XC_MethodHook.MethodHookParam;
import com.power.android.dance.DreamBridge.CopyOnWriteSortedSet;
import com.power.android.dance.callbacks.XC_LayoutInflated;
import com.power.android.dance.callbacks.XC_LayoutInflated.LayoutInflatedParam;
import com.power.android.dance.callbacks.XCallback;
import xposed.dummy.XResourcesSuperClass;
import xposed.dummy.XTypedArraySuperClass;

import static com.power.android.dance.XposedHelpers.decrementMethodDepth;
import static com.power.android.dance.XposedHelpers.findAndHookMethod;
import static com.power.android.dance.XposedHelpers.getIntField;
import static com.power.android.dance.XposedHelpers.getLongField;
import static com.power.android.dance.XposedHelpers.getObjectField;
import static com.power.android.dance.XposedHelpers.incrementMethodDepth;
...
public class XResources extends XResourcesSuperClass {
	...
	private static String getPackageName(String resDir) {
		...
		if (pkgInfo != null && pkgInfo.packageName != null) {
			Log.w(DreamBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { DreamBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						DreamBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					DreamBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				DreamBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			DreamBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
